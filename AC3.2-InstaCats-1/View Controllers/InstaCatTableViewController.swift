...
class InstaCatTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        guard let instaCatsURL: URL = self.getResourceURL(from: instaCatJSONFileName),
            let instaCatData: NSData = self.getData(from: instaCatsURL) as NSData?,
    ...
    internal func getInstaCats(from jsonData: Data) -> [InstaCat]? {
        var appendCats = [InstaCat]() //need local var
                ...
                if let catData = catDict["cats"] {
                    for cats in catData {
                        if let name = cats["name"] as? String,
                            ...
                            let instagramURL = cats["instagram"] as? String {
                            
                            guard let id = Int(idString), let instagramURL = URL(string: instagramURL) else {return nil}
                            
                            let instagramCat = InstaCat(name: name, catID: id, instagramURL: instagramURL)
                            appendCats.append(instagramCat)
